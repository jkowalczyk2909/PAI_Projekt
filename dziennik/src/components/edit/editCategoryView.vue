<template>
  <div id="container">
    <h1 class="margin-top">Edytuj kategorię</h1>
...
export default {
  ...
  mounted(){    
    http.get("/api/category/"+this.$route.params.id).then(result=>{
  ...
  methods:{
    submitForm(){
      http.put("/api/category/"+this.$route.params.id,{
        name: this.name,
        weight: this.weight,
        color: this.color
      }).then(results => {
    ...
    usun(){
      if (confirm("Czy na pewno chcesz usunąć?")){
        http.delete("/api/category/"+this.$route.params.id).then(results => {
          let json = results.data;
