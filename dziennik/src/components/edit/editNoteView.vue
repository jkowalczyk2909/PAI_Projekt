<template>
  <div id="container">
    <h1 class="margin-top">Edytuj ocenę</h1>
...
export default {
  ...
  async mounted(){
    ...
    await http.get("/api/note/"+this.$route.params.id).then(results => {
      let json = results.data;

      this.note = json.value;
      this.category = json.category_id;
      this.comment = json.title;
      this.subject_id = json.subject_id;
      this.student_id = json.user_id;
    });

    await http.get("/api/subject_name/"+this.student_id+"/"+this.subject_id).then(results => {
      let json = results.data;
  ...
  methods:{
    submitForm(){
      http.put("/api/note/"+this.$route.params.id,{
        value: this.note,
    ...
    usun(){
      if (confirm("Czy na pewno chcesz usunąć?")){
        http.delete("/api/note/"+this.$route.params.id).then(results => {
          let json = results.data;
  ...
  watch: {
    ...
    subject_id(newValue){
      this.subject_id = newValue;
    },
    student_id(newValue){
      this.student_id = newValue;
    }
  }
}
</script>